...
pros_cat <- read.csv('prospect_catchers.csv') #From fangraphs THE BOARD
pit_mov <- read.csv('pitch_movement.csv') #Fome baseball savant export csv
...
command <- mlb_stats(stat_type = 'season', player_pool = 'all', stat_group = 'pitching', season = '2022')
command <- command %>% filter(position_type == 'Pitcher', games_started > 1) %>% select(outs)
...
# 
# modal <- data.frame(matrix(ncol=12, nrow=1))
# colnames(modal) <- c('Name', 'Hit', 'Game', 'Raw', 'Spd', 'Fld', 'Arm', 'FB', 'SL', 'CB', 'CH', 'CMD' )
# 
# selected <- "Corbin Carroll"
# 
# 
